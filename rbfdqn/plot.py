...
problems_name = [
    'Pendulum', 'LunarLander', 'Bipedal', 'Hopper', 'Cheetah', 'Ant', 'InvertedDoublePendulum',
    'InvertedPendulum', 'Reacher'
]
...
#setting_li=[0]
labels = [
    '100 updates', '200 updates', '200 updates slow target', '200 updates slowest target',
    '200 updates slowest! target', '200 updates less target updates',
...
for problem in [0]:
    ...
    for setting in [0]:  #in range(5, 10):
        ...
        #print([len(x) for x in li])
        li = truncate(li)
        print(hyper_parameter_name, len(li[0]), numpy.mean(numpy.mean(li, axis=0)[-1:]),
