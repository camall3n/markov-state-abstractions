...
d = d.query(
    "(temperature == '0.5' and learning_rate == '0.0001' and features == 'markov' and markov_coef in ['0.03'])"
)
...
for domain in best_of.keys():
    ...
    } for (features, temperature, learning_rate) in best_of[domain]] + take_all

def build_query(filters):
    query = ' or '.join([
        '(' +
        ' and '.join([field + ' == "{}"'.format(value)
                      for (field, value) in filter_.items()]) + ')' for filter_ in filters
    ])
    return query
...
for domain in domains:
    ...
    data = data.query("features != 'markov' or markov_coef in [0, 0.03]")

    data.loc[data.features == 'markov', 'features'] = 'Markov'
    data.loc[data.features == 'expert', 'features'] = 'Expert'
    data.loc[data.features == 'visual', 'features'] = 'Visual'
    data = data.rename(columns={'episode': 'Steps (x1000)', 'reward': 'Reward', 'features': 'Features'})

    data.groupby('Features')['seed_number'].nunique()

    p = sns.color_palette('viridis', n_colors=len(data['Features'].unique()), desat=0.5)
    ...
    g = sns.relplot(
        ...
        y='Reward',
        hue='Features',
        hue_order=['Markov', 'Visual', 'Expert'],
        style='Features',
        # col='data_aug',
        style_order=['Markov', 'Visual', 'Expert'],
        kind='line',
        # units='seed_number',
        # estimator=None,
        palette=p)
